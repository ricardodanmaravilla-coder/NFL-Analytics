"""Spread ATS v5: causal ML correction around the no-vig market baseline. Validation only; 2026 excluded."""
import os
import numpy as np
import pandas as pd
...
def training(games,pbp):
 ...
 f['market_spread_home']=pd.to_numeric(f.spread_line,errors='coerce')
 mh=[]
 for a,z in zip(f.home_spread_odds,f.away_spread_odds):
  try: mh.append(nv(a,z)[0])
  except Exception: mh.append(np.nan)
 f['market_p_home']=mh; cm=pd.to_numeric(f.result,errors='coerce')-f.market_spread_home
 f['home_cover']=np.where(cm.abs()<1e-9,np.nan,(cm>0).astype(float)); f['market_residual_target']=f.home_cover-f.market_p_home
 return b,ordered(f),cols+['market_spread_home','market_p_home']

def row(b,pbp,w,h,a,g,line,mkt,cols):
...
def main():
 ...
 raw=ordered(raw); ppfile='data/historico_nfl_pbp_team_game.csv'; pbp=pd.read_csv(ppfile) if os.path.exists(ppfile) else pd.DataFrame(); allrows=[]; picks=[]
 for s in [2023,2024,2025]:
  for w in sorted(pd.to_numeric(raw[raw.season==s].week,errors='coerce').dropna().astype(int).unique()):
   past=historico_antes(raw,s,w); pp=historico_antes(pbp,s,w) if not pbp.empty else pd.DataFrame(); b,tr,cols=training(past,pp); tr=ordered(tr.dropna(subset=cols+['market_residual_target']))
   if len(tr)<250:continue
   model=RandomForestRegressor(n_estimators=500,max_depth=6,min_samples_leaf=20,max_features=.65,random_state=117,n_jobs=1).fit(tr[cols],tr.market_residual_target)
   ...
   for _,g in week.iterrows():
    ...
    mh,ma=nv(ho,ao); x=row(b,pp,w,h,a,g,line,mh,cols)
    if x is None:continue
    correction=float(model.predict(x)[0]); ph=float(np.clip(mh+correction,.01,.99)); cover=float(g.result)-float(line)
    if abs(cover)<1e-9:continue
    y=int(cover>0); allrows.append({'season':s,'week':w,'game_id':g.game_id,'home':h,'away':a,'spread_line':float(line),'p_home':ph,'market_p_home':mh,'correction':correction,'home_cover':y})
    for side,prob,odd,imp,ln,win in [('H',100*ph,float(ho),mh,-float(line),y),('A',100*(1-ph),float(ao),ma,float(line),1-y)]:
     ...
     if prob>=MIN_P and edge>=MIN_EDGE and ev>=MIN_EV:
      role='FAVORITE' if ln<0 else ('UNDERDOG' if ln>0 else 'PICKEM'); picks.append({'season':s,'week':w,'game_id':g.game_id,'side':side,'role':role,'line':ln,'probability':prob,'market_probability':100*imp,'edge':edge,'ev':ev,'odds':odd,'correction':correction,'win':win,'return':dec(odd)-1 if win else -1.})
 ...
 print('\n=== ALL OOS: MARKET BASELINE + ML CORRECTION ===')
 for s,g in A.groupby('season'):
  ...
  print(s,{'n':len(g),'brier_model':round(brier_score_loss(y,p),4),'brier_market':round(brier_score_loss(y,mp),4),'logloss_model':round(log_loss(y,p,labels=[0,1]),4),'logloss_market':round(log_loss(y,mp,labels=[0,1]),4),'mean_correction_pp':round(100*g.correction.mean(),2)})
 if O.empty:
  print('\nNO PICKS passed fixed 58/3/3 filters'); return
 print('\n=== FIXED FILTER PICKS ==='); print({'n':len(O),'winrate':round(100*O.win.mean(),2),'roi':round(100*O['return'].mean(),2),'avg_p':round(O.probability.mean(),2)})
