...
2026 queda fuera. NFL_VALIDATION_SEASON permite dividir la ejecución sin cambiar
la lógica, filtros ni muestra de cada temporada.
"""
import os
import numpy as np
import pandas as pd
...
def grade(market,side,hs,aws,line):
    ...
    adj=(margin+line) if market=='SPREAD' and side=='H' else ((-margin+line) if market=='SPREAD' else ((total-line) if side=='O' else (line-total)))
...
def main():
    ...
    pbp_path='data/historico_nfl_pbp_team_game.csv';pbp=pd.read_csv(pbp_path) if os.path.exists(pbp_path) else pd.DataFrame();rows=[]
    requested=os.getenv('NFL_VALIDATION_SEASON','').strip()
    seasons=[int(requested)] if requested else [2023,2024,2025]
    if any(s not in (2023,2024,2025) for s in seasons):raise SystemExit('NFL_VALIDATION_SEASON must be 2023, 2024 or 2025')
    print('VALIDATION_SEASONS',seasons,'2026_EXCLUDED',True,flush=True)
    for season in seasons:
        sg=raw[raw['season']==season]
        for week in sorted(pd.to_numeric(sg['week'],errors='coerce').dropna().astype(int).unique()):
            print('WALK_FORWARD',season,week,flush=True)
            past=historico_antes(raw,season,week);pp=historico_antes(pbp,season,week) if not pbp.empty else pd.DataFrame();model=MoneylineRuntime()
            if not model.entrenar(past,pp):continue
            elo=MotorELONFL();elo.actualizar_ratings(past);wk=sg[pd.to_numeric(sg['week'],errors='coerce')==week]
            for _,g in wk.iterrows():
                ...
                if not pd.isna(hm) and not pd.isna(am) and ph is not None and eh is not None:
                    if max(ph,elo_h,eh)-min(ph,elo_h,eh)<=MAX_DISAGREE and all((x>=50)==(ph>=50) for x in [elo_h,eh]):add(rows,season,week,g.get('game_id'),'ML','H',ph,eh,hm,am,float(hs),float(aws))
                    if max(pa,100-elo_h,ea)-min(pa,100-elo_h,ea)<=MAX_DISAGREE and all((x>=50)==(pa>=50) for x in [100-elo_h,ea]):add(rows,season,week,g.get('game_id'),'ML','A',pa,ea,am,hm,float(hs),float(aws))
