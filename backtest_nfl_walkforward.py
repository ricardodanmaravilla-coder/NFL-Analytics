...
def select_pick(primary, supports, odd_self, odd_other):
    ...
    if p < 54 or edge < 3 or ev < 3:
        return None
    return {"p": p, "decimal": d, "edge": edge, "ev": ev}
...
def evaluate_season(raw, pbp, target_season):
    ...
    for week in weeks:
        ...
        for _, r in wk.iterrows():
            ...
            # Backtest usa clima prepartido histórico almacenado; nunca forecast posterior.
            ...
            p_emp_h, p_emp_a = two_way(emp["Moneyline"].get("Gana Local"), emp["Moneyline"].get("Gana Visita"))
            if p_emp_h is None:
                continue

            y_home = int(float(r["home_score"]) > float(r["away_score"]))
            probability_rows.append({
                "season": target_season, "week": week, "game_id": r.get("game_id"),
                "y": y_home, "p": p_primary / 100.0,
                "brier": (p_primary / 100.0 - y_home) ** 2,
                "logloss": logloss(y_home, p_primary / 100.0),
            })

            hm, am = r.get("home_moneyline"), r.get("away_moneyline")
            if pd.isna(hm) or pd.isna(am):
                continue
            home_pick = select_pick(p_primary, [p_elo, p_emp_h], hm, am)
            away_pick = select_pick(100 - p_primary, [100 - p_elo, p_emp_a], am, hm)
...
def main():
    ...
    results, all_probs, all_bets = [], [], []
    # 2023-2025 permiten histórico previo suficiente. 2026 queda intocable.
    for season in [2023, 2024, 2025]:
        r, p, b = evaluate_season(raw, pbp, season)
        results.append(r)
        all_probs.append(p)
        all_bets.append(b)
    ...
    # Guardrails estructurales, no se fuerza ROI positivo para evitar tuning al test.
