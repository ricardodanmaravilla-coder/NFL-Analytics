"""Diagnostico OOS del Direct ATS por temporada, rol, linea y probabilidad.
...
def stats(g):
    return pd.Series({'n':len(g),'winrate':g.win.mean(),'roi':g['return'].mean(),'avg_p':g.probability.mean(),'avg_line':g.line.mean()})
...
def main():
    ...
    # probability esta expresada en porcentaje (58-100), no en escala 0-1.
    df['p_bucket']=pd.cut(df.probability,[58,60,63,66,70,101],labels=['58-60','60-63','63-66','66-70','70+'],include_lowest=True)
    tables=[]
    ...
    drift=df.groupby('season').agg(n=('win','size'),avg_p=('probability','mean'),sd_p=('probability','std'),avg_abs_line=('abs_line','mean'),favorite_share=('role',lambda x:(x=='FAVORITE').mean()),avg_odds=('odds','mean'),avg_edge=('edge','mean'),avg_ev=('ev','mean')).reset_index()
